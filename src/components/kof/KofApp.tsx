...
import { KofTable } from "@/components/kof/KofTable";
import { EmptyState } from "@/components/EmptyState";
import { ErrorMessage } from "@/components/ErrorMessage";
import { ExcelUploader } from "@/components/ExcelUploader";
import { Button } from "@/components/ui/button";
import {
  applyKofInput2ToWorkbook,
  buildT5AndDifferens,
  parseKofFile,
  parseKofInput2File,
} from "@/lib/kof/parser";
import type { KofParseError, KofRow, KofWorkbook } from "@/lib/kof/types";
import { cn } from "@/lib/utils";

type AppState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "error"; error: KofParseError }
  | {
      status: "success";
      data: KofWorkbook;
...
function buildOrdernrList(rows: KofRow[]): string {
...
export function KofApp() {
  const [state, setState] = useState<AppState>({ status: "idle" });
  const [ordernrCopied, setOrdernrCopied] = useState(false);
  const [input2Error, setInput2Error] = useState<KofParseError | null>(null);
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    const result = await parseKofFile(file);
  ...
  const handleInput2 = useCallback(
    async (file: File) => {
      ...
      const result = await parseKofInput2File(file);
      ...
      const merged = applyKofInput2ToWorkbook(
        previous,
        result.data.rows,
      ...
      return {
        ...
        data: {
          ...
          rows: prev.data.rows.map((row) =>
            row.id === rowId
              ? { ...row, ...buildT5AndDifferens(t5, row.pris) }
              : row
          ),
        },
      };
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1800px] flex-1 flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-2">
          ...
          <p className="max-w-2xl text-[#b8b8b8]">
            Ladda upp CSV-filen, kopiera Ordernr, och ladda upp Input 2 för att
            fylla kolumnen T5 (resurs 3006).
          </p>
        </div>
      ...
      {(state.status === "idle" || state.status === "error") && (
        <ExcelUploader
          title="Input 1 — Ladda upp CSV"
          acceptCsv
          onFileSelected={(file) => void handleFile(file)}
          hint={
            <>
              CSV med kolumner som{" "}
              <span className="text-white">Avsdatum</span>,{" "}
              <span className="text-white">Från_ort</span>,{" "}
              <span className="text-white">Till</span>,{" "}
              <span className="text-white">Ref</span> (FRS / Ordernr) m.fl.
            </>
          }
        />
      )}
      ...
      {state.status === "loading" && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-[#3a3a3a] bg-[#242424] px-6 py-20 shadow-[0_4px_20px_rgba(0,0,0,0.25)]">
          <Loader2
            ...
            aria-hidden
          />
          <p className="text-white">Bearbetar CSV-fil…</p>
          <p className="text-sm text-[#b8b8b8]">Läser KOF Input 1</p>
        </div>
      )}
      ...
      {state.status === "success" && (
        <div className="space-y-6">
          ...
          <KofTable rows={state.data.rows} onT5Change={handleT5Change} />
        </div>
      )}
      ...
      {state.status === "idle" && (
        <EmptyState
          ...
          description="Ladda upp en CSV-fil för Input 1 för att visa KOF-rader och kopiera Ordernr."
        />
      )}
    </div>
  );
