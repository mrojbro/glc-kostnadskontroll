...
type AppState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "error"; error: KrickosParseError }
  | { status: "success"; data: KrickosWorkbook; fileName: string };

export function KrickosApp() {
  const [state, setState] = useState<AppState>({ status: "idle" });

  const handleFile = useCallback(async (file: File) => {
    setState({ status: "loading" });

    const result = await parseKrickosFile(file);
    ...
    <div className="mx-auto flex w-full max-w-[1800px] flex-1 flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      {state.status === "loading" && (
        <div className="flex flex-col items-center justify-center gap-3 rounded-2xl border border-[#3a3a3a] bg-[#242424] px-6 py-20 shadow-[0_4px_20px_rgba(0,0,0,0.25)]">
          ...
          <p className="text-sm text-[#b8b8b8]">Läser Krickos-underlag</p>
        </div>
      )}
      ...
      {state.status === "idle" && (
        <EmptyState
          ...
          description="Ladda upp en Excel-arbetsbok för att visa 2215 Krickos."
        />
      )}
    </div>
  );
